...
std::string_view
Skysight::GetRegionTimeZone() const noexcept
{
...
RoughTimeDelta
Skysight::GetForecastDisplayOffset(time_t utc_time) const noexcept
{
...
bool
Skysight::IsAutoUpdateEnabled() const noexcept
{
  return CommonInterface::GetComputerSettings().weather.skysight.auto_update;
}

void
Skysight::OnAutoUpdateChanged() noexcept
{
...
bool
Skysight::IsLiveViewUpdating(std::string_view layer_id) const noexcept
{
...
bool
Skysight::HasDownloadActivity() const noexcept
{
...
bool
Skysight::SelectAutomaticForecastTime(std::string_view id, bool download)
{
  auto *layer = api->GetLayer(id);
  if (layer == nullptr || !api->IsSelectedLayer(id) ||
      layer->SupportsLiveTiles())
...
bool
Skysight::SetLayerActive(std::string_view id, bool request_update)
{
  auto *layer = api->GetLayer(id);
  if (layer == nullptr)
    return false;
...
void
Skysight::ApplyPageOverlay(const PageLayout &page) noexcept
{
  try {
...
void
Skysight::OnForecastProgressCancelled() noexcept
{
