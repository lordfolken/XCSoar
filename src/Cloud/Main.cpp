...
public:
     ...
     ogn_client(event_loop, cares_channel, ogn_traffic)
  {
    ...
    // OGN client - use full feed (port 10152) without filter
    ogn_client.SetServer("aprs.glidernet.org", 10152);
    ogn_client.SetFilter("");  // No filter for full feed
  ...
  void SetOGNCallsign(const char *callsign) {
    ogn_client.SetCallsign(callsign);
  }

  void SetOGNPasscode(int passcode) {
    ogn_client.SetPasscode(passcode);
  }
...
try {
  ...
  const char *ogn_callsign = nullptr;
  int ogn_passcode = -1;

  // Parse command line arguments
  for (int i = 1; i < argc; i++) {
    if (strncmp(argv[i], "--ogn-callsign=", 15) == 0) {
      ogn_callsign = argv[i] + 15;
    } else if (strncmp(argv[i], "--ogn-passcode=", 15) == 0) {
      ogn_passcode = atoi(argv[i] + 15);
    } else if (strcmp(argv[i], "--help") == 0 || strcmp(argv[i], "-h") == 0) {
      cerr << "Usage: " << argv[0] << " DBPATH [OPTIONS]" << endl;
      cerr << "Options:" << endl;
      cerr << "  --ogn-callsign=CALLSIGN   APRS callsign (e.g., XC0AR)" << endl;
      cerr << "  --ogn-passcode=PASSCODE   APRS passcode (use -1 for unverified)" << endl;
      cerr << "  --help, -h                Show this help message" << endl;
      cerr << endl;
      cerr << "Environment variables:" << endl;
      cerr << "  OGN_CALLSIGN              APRS callsign (overridden by --ogn-callsign)" << endl;
      cerr << "  OGN_PASSCODE              APRS passcode (overridden by --ogn-passcode)" << endl;
      return EXIT_SUCCESS;
    } else if (argv[i][0] != '-') {
  ...
  if (ogn_callsign == nullptr) {
    const char *env_callsign = std::getenv("OGN_CALLSIGN");
    if (env_callsign != nullptr && env_callsign[0] != '\0') {
      ogn_callsign = env_callsign;
    }
  }

  if (ogn_passcode == -1) {
    const char *env_passcode = std::getenv("OGN_PASSCODE");
    if (env_passcode != nullptr && env_passcode[0] != '\0') {
      ogn_passcode = atoi(env_passcode);
    }
  }
  ...
  // Configure OGN client
  if (ogn_callsign != nullptr) {
    server.SetOGNCallsign(ogn_callsign);
    cout << "OGN: Using callsign from config: " << ogn_callsign << endl;
    
    // Auto-calculate passcode if not provided
    if (ogn_passcode == -1) {
      int calculated_passcode = OGNClient::CalculatePasscode(ogn_callsign);
      if (calculated_passcode != -1) {
        server.SetOGNPasscode(calculated_passcode);
        cout << "OGN: Auto-calculated passcode from callsign: " 
             << calculated_passcode << endl;
      } else {
        cout << "OGN: Warning: Could not calculate passcode, using unverified (-1)" << endl;
      }
    } else {
      server.SetOGNPasscode(ogn_passcode);
      cout << "OGN: Using passcode from config: " << ogn_passcode << endl;
    }
  } else if (ogn_passcode != -1) {
    // Passcode provided but no callsign - use default callsign
    server.SetOGNPasscode(ogn_passcode);
    cout << "OGN: Using passcode from config: " << ogn_passcode << endl;
  }
