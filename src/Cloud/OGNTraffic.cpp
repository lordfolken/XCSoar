...
OGNTraffic &
OGNTrafficContainer::Make(const tstring &device_id,
                          ...
                          FlarmTraffic::AircraftType aircraft_type)
{
  ...
  if (i != device_map.end()) {
    ...
    if (location != traffic.location) {
      ...
      rtree.remove(ptr);
      traffic.Update(location, altitude, track, speed, climb_rate,
  ...
  if (StringStartsWith(id_str, _T("FLR"))) {
    ...
    // PilotAware device: "PAW407B1B" -> parse "407B1B" as hex (ICAO 24-bit address)
#ifdef _UNICODE
    wchar_t *endptr = nullptr;
    const wchar_t *hex_start = id_str + 3;
#else
...
{
  const auto q = boost::geometry::index::intersects(
    BoostRangeBox(location, range));
