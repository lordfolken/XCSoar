    ...
    boost::intrusive::set_base_hook<boost::intrusive::link_mode<boost::intrusive::normal_link>>
{
  ...
  OGNTraffic(unsigned _id, const tstring &_device_id, unsigned _flarm_id = 0)
    :device_id(_device_id), id(_id), flarm_id(_flarm_id),
     ...
     altitude(-1), track(0), speed(0), climb_rate(0),
     turn_rate(0), aircraft_type(FlarmTraffic::AircraftType::UNKNOWN) {}

  void Update(const GeoPoint &_location, int _altitude,
              unsigned _track, unsigned _speed, int _climb_rate,
              double _turn_rate = 0,
              FlarmTraffic::AircraftType _aircraft_type = FlarmTraffic::AircraftType::UNKNOWN) {
    location = _location;
    altitude = _altitude;
    track = _track;
    speed = _speed;
...
class OGNTrafficContainer {
  typedef boost::geometry::index::rtree<OGNTrafficPtr, boost::geometry::index::rstar<16>,
                                        OGNTrafficIndexable> Tree;

  typedef boost::intrusive::list<OGNTraffic,
                                 boost::intrusive::constant_time_size<false>> List;

  typedef boost::intrusive::set<OGNTraffic,
                                boost::intrusive::compare<OGNTraffic::IdCompare>,
...
public:
   ...
   */
  OGNTraffic &Make(const tstring &device_id,
