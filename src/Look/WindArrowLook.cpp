...
WindArrowLook::Initialise(const Font &_font, bool inverse)
{
  ...
  Color color_extern(LightColor(COLOR_BLUE)); // a light blue
  arrow_brush_extern.Create(IsDithered()
                                ? color_extern
                                : ColorWithAlpha(color_extern, ALPHA_OVERLAY));
  Color color_instantaneous(LightColor(COLOR_GREEN)); // a light green
  arrow_brush_instantaneous.Create(
      IsDithered() ? color_instantaneous
                   : ColorWithAlpha(color_instantaneous, ALPHA_OVERLAY));
